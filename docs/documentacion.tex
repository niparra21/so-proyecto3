...
Para revisar el comportamiento con archivos de diferentes tama\~{n}os, se definio una
prueba manual de rendimiento que puede ejecutarse en ambiente Unix. La prueba mide
carga, descarga y verificacion de integridad usando archivos binarios generados con
\texttt{dd}.
...
dd if=/dev/urandom of=tmp-perf/in/1k.bin bs=1024 count=1
dd if=/dev/urandom of=tmp-perf/in/1m.bin bs=1024 count=1024
dd if=/dev/urandom of=tmp-perf/in/5m.bin bs=1024 count=5120
./aws-s3_server 19191 > tmp-perf/server.log 2>&1 &
SERVER_PID=$!
sleep 1
./aws-s3 --port 19191 mb s3://perf
/usr/bin/time -p ./aws-s3 --port 19191 cp tmp-perf/in s3://perf/data/ --recursive
/usr/bin/time -p ./aws-s3 --port 19191 cp s3://perf/data/ tmp-perf/out --recursive
cmp tmp-perf/in/1k.bin tmp-perf/out/1k.bin
cmp tmp-perf/in/1m.bin tmp-perf/out/1m.bin
cmp tmp-perf/in/5m.bin tmp-perf/out/5m.bin
...
La sincronizacion compara existencia y tama\~{n}o de archivos. Esta decision mantiene el
dise\~{n}o simple y suficiente para el alcance del proyecto.
